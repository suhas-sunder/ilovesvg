...
| `ILSVG-DEF-003` | `/webp-to-svg-for-cricut`: local and shared cards used `cricut-clean-cut`. **Medium. Status: fixed in the converter-state remediation milestone.** | The route-local default is now `webp-cricut-clean-cut`; the shared card retains `cricut-clean-cut`. Active, pin, history, default, and clicked metadata resolve exactly. | `app/routes/webp-to-svg-for-cricut.tsx`; focused preset audit; parity collision row; browser matrix. | Corrected in this milestone; cut settings, output, presentation, and default behavior are preserved. |
| `ILSVG-DEF-004` | `/jpeg-to-svg-for-cricut`: duplicate reachable `cricut-clean-cut` identity. **Medium. Status: fixed in the converter-state remediation milestone.** | The route-local default is now `jpeg-cricut-clean-cut`; the shared identity remains unchanged. Submitted ID/label metadata and history navigation are exact. | `app/routes/jpeg-to-svg-for-cricut.tsx`; focused preset audit; parity collision row; browser matrix. | Corrected in this milestone; both effective settings and outputs are preserved. |
...
| `ILSVG-DEF-008` | `/png-to-svg-for-shopify` exposes `Etsy PNG - Accurate trace (default)`. **Medium. Status: fixed in the public-content/schema remediation milestone.** | Misleading platform default; output changes only through the selected preset/settings; no separate history corruption shown. The Shopify route now resolves Shopify-specific visible labels from the unchanged preset IDs and values. | `app/routes/png-to-svg-for-etsy.tsx`; focused public-copy audit; converter-parity audit. | Corrected in this milestone; preserve the established default, ordering, effective values, history ownership, and output. |
| `ILSVG-DEF-009` | `/svg-to-favicon-generator` visible breadcrumb and BreadcrumbList target `/svg-favicon-generator`, which returns 404. **Medium. Status: fixed in the public-content/schema remediation milestone.** | Broken navigation/schema; no generated package or history impact. Visible and schema breadcrumbs now use the registered `/svg-to-favicon-generator` route. | `app/routes/svg-to-favicon-generator.tsx`; focused public-copy audit; runtime schema audit. | Corrected in this milestone; generator inputs, package manifest/snippet, canonical, and route remain unchanged. |
| `ILSVG-DEF-010` | `/svg-stroke-width-editor` targets invalid `/svg-stroke-width-adjust`. **Medium. Status: fixed in the public-content/schema remediation milestone.** | Broken visible/schema navigation; no SVG output/history corruption shown. Visible and schema breadcrumbs now use the registered `/svg-stroke-width-editor` route. | `app/routes/svg-stroke-width-editor.tsx`; focused public-copy audit; runtime schema audit. | Corrected in this milestone; editor behavior and canonical remain unchanged. |
| `ILSVG-DEF-011` | `/svg-flip-and-rotate-editor` targets invalid `/svg-flip-rotate-editor`. **Medium. Status: fixed in the public-content/schema remediation milestone.** | Broken visible/schema navigation; no transformed output/history corruption shown. Visible and schema breadcrumbs now use the registered `/svg-flip-and-rotate-editor` route. | `app/routes/svg-flip-and-rotate-editor.tsx`; focused public-copy audit; runtime schema audit. | Corrected in this milestone; transform behavior and canonical remain unchanged. |
| `ILSVG-DEF-012` | `/sketch-to-svg-converter` renders rental-agreement/budgeting content. **Low. Status: fixed in the public-content/schema remediation milestone.** | Confusing unrelated public guidance; no output/history impact. It was replaced with concise sketch-photo, contrast, trace-style, and detail-review guidance grounded in the existing workflow. | `app/routes/sketch-to-svg-converter.tsx`; focused public-copy audit; rendered-route inspection. | Corrected in this milestone; converter behavior and useful sketch guidance remain unchanged. |
| `ILSVG-DEF-013` | Raster-converter UI exposes internal Potrace/VTracer/parity/backend notes. **Low. Status: fixed in the public-content/schema remediation milestone.** | Internal terminology confuses users; no measured output/history impact. Public method labels, warnings, progress, preset help, and conversion-limit explanations now describe user-visible outcomes while internal identifiers and diagnostics remain intact. | `TraceOutputPanel.tsx`, `useHybridTraceFetcher.ts`, `vtracer.worker.ts`, shared preset/help components, relevant converter routes; focused public-copy audit. | Corrected in this milestone; engine choice, algorithms, limits, output, and diagnostic identifiers remain unchanged. |
| `ILSVG-DEF-014` | Public converter/sitemap content includes SEO/editorial terminology such as keyword cluster and search intent. **Low. Status: fixed in the public-content/schema remediation milestone.** | Editorial implementation language is user-visible; no output/history impact. Confirmed phrases now use route-specific navigation and workflow wording. | `OtherToolsLinks.tsx`, `app/routes/sitemap.tsx`; focused public-copy audit; rendered-route inspection. | Corrected in this milestone; useful route links remain and the All Tools implementation is untouched. |
| `ILSVG-DEF-015` | Favicon guidance simultaneously describes raster input support and an SVG-only/non-vectorizing limitation. **Medium. Status: fixed in the public-content/schema remediation milestone.** | Users receive contradictory accepted-input guidance; package output/history is not shown corrupt. Guidance now states that SVG retains scalable source quality before icon generation while PNG, JPG, and WebP inputs are resized directly and are not vectorized. | `OtherToolsLinks.tsx`, `app/routes/svg-to-favicon-generator.tsx`; focused public-copy audit; converter-parity audit. | Corrected in this milestone; accepted inputs, package manifest/snippet, ICO sizes, background behavior, and route-specific workflows remain unchanged. |
