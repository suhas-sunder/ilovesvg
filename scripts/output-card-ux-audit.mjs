...
checks.push(
  ...
  assertIncludes(outputPanel, "data-collapse-state", "shared output panel exposes collapse state"),
    ...
    assertIncludes(outputPanel, "syncOwnedCacheKeys", "shared output panel removes appearance state only after output ownership ends"),
    assertIncludes(outputPanel, "releaseOwnedCacheKeys", "shared output panel releases component-owned appearance state on unmount"),
    ...
    assertIncludes(advancedSettingsPanel, "DEFAULT_TOP_LEVEL_SETTINGS_GROUP_OPEN = true", "top-level settings groups default open"),
    assertIncludes(advancedSettingsPanel, "setLiveTopOpen] = React.useState(", "Live Preview Edits initializes through React state"),
    assertIncludes(advancedSettingsPanel, "setConvertTopOpen] = React.useState(", "Click To Convert initializes through React state"),
    assertNotIncludes(advancedSettingsPanel, "setLiveTopOpen] = React.useState(false)", "Live Preview Edits must not default closed"),
    assertNotIncludes(advancedSettingsPanel, "setConvertTopOpen] = React.useState(false)", "Click To Convert must not default closed"),
    assertNotIncludes(advancedSettingsPanel, "localStorage", "top-level settings groups ignore stale localStorage state"),
  ...
  assertIncludes(browserSmoke, "topLevelSettingsOpen", "browser smoke verifies top-level settings groups default open"),
...
if (bespokeOutputPanel) {
  checks.push(
    ...
    assertIncludes(bespokeOutputPanel, "syncOwnedCacheKeys", "bespoke output panel removes appearance state only after output ownership ends"),
    assertIncludes(bespokeOutputPanel, "releaseOwnedCacheKeys", "bespoke output panel releases component-owned appearance state on unmount"),
