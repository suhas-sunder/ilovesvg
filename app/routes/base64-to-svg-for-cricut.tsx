...
export function meta({}: Route.MetaArgs) {
  const title = "Base64 to SVG for Cricut - Decode Base64 SVG Online";
  const description =
    "Convert Base64 SVG code, SVG data URLs, and encoded SVG strings into downloadable Cricut-ready SVG files. Decode, clean, preview, copy, and export SVG files online.";
  const canonical = "https://www.ilovesvg.com/base64-to-svg-for-cricut";
...
const RATE_LIMIT_ROUTE_KEY = "base64-to-svg-for-cricut";
...
const PRESETS: Preset[] = [
  {
    ...
    help: "Single-color trace preset from the main converter. Best for clean black lineart and non-colored cut paths.",
    settings: {
      rasterMode: "single",
      preprocess: "none",
      threshold: 224,
  ...
  {
    id: "single-color-cut",
    label: "Single-color cut file",
    help: "For vinyl-style cut files where all paths should become one solid color.",
    settings: {
  ...
  },
  {
    ...
    help: "For checking cut paths and outlines before uploading to Cricut Design Space.",
    settings: {
  ...
  {
    id: "keep-print-then-cut",
    label: "Keep print then cut art",
    help: "Preserves colors and embedded image references when the SVG is for print then cut.",
    settings: {
      colorMode: "preserve",
      removeScripts: true,
      removeMetadata: true,
...
/* ========================
   Page
======================== */
export default function Base64ToSvgForCricut({}: Route.ComponentProps) {
  ...
  React.useEffect(() => {
    ...
    setHistory((prev) =>
      [
        ...
        ...prev.filter((item) => item.svg !== decoded.svg),
      ].slice(0, 10),
    );
  }, [
    decoded.displaySvg,
  ...
  function downloadSvgContent(svg: string) {
    ...
    link.href = url;
    link.download = "cricut-design.svg";
  ...
  function downloadCsv() {
    ...
    link.href = url;
    link.download = "base64-to-svg-for-cricut-report.csv";
  ...
  function updateDecodedLayer(layerId: string, patch: Partial<LayerState>) {
    ...
    const svg = buildClientLayeredSvg({
      width: base.width,
      height: base.height,
      layers,
      transparent: settings.backgroundMode !== "solid",
        ...
        <div className="mx-auto max-w-[1180px] px-4">
          ...
          <section className="grid grid-cols-1 items-start gap-4 sm:pt-6 md:grid-cols-2 lg:pb-8 lg:pt-0">
            <div className="min-w-0 overflow-hidden rounded-xl bg-white p-4 sm:border sm:border-slate-200 sm:shadow-sm">
              ...
              <div className="mt-3 min-w-0">
                ...
                >
                  <span className="inline-flex items-center gap-2">
                    Additional decode and Cricut settings
                  </span>
                ...
                {showAdvanced && (
                  ...
                  >
                    ...
                    <Field label="Add Cricut note">
                      <input
                        type="checkbox"
                        checked={settings.addCricutNote}
                        onChange={(event) =>
                          update("addCricutNote", event.target.checked)
                        }
                        className="h-4 w-4 cursor-pointer accent-[#0b2dff]"
                      />
                    </Field>
              ...
              >
                Tips for Base64 SVGs and Cricut
                ...
                >
                  <ul className="list-disc space-y-1 pl-5">
                    ...
                    <li>
                      ...
                      for Cricut-style layered designs.
                    </li>
                    <li>
                      Use single-color trace presets for vinyl-style cut paths.
                    </li>
                    <li>
            ...
            <div className="h-full max-h-[124.25em] min-w-0 overflow-auto rounded-xl border border-slate-200 bg-slate-600 p-4 shadow-sm">
              ...
              {rasterConversionBusy && !decoded.error && (
                <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm leading-6 text-sky-900">
                  Converting the Base64 image into layered SVG cut paths.
                </div>
              )}
...
function decodeAndCleanSvg(settings: Settings): DecodedResult {
  ...
  try {
    ...
    const displaySvg = svgToDataUrl(cleaned.svg);

    const layers = buildEditableSvgLayers(cleaned.svg);
...
function buildEditableSvgLayers(svg: string): LayerState[] | undefined {
  ...
  if (!pathTags.trim()) return undefined;
...
function cleanSvg(svgText: string, settings: Settings) {
  ...
  if (settings.addCricutNote) {
    const title = doc.createElementNS("http://www.w3.org/2000/svg", "title");
    title.textContent = "Decoded SVG for Cricut";
...
function buildWarning({
  ...
  if (hasRasterImage && !settings.removeRasterImages) {
    return "This SVG contains embedded raster images. Cricut may treat those parts as print artwork, not cut paths.";
  }

  if (hasText) {
    return "This SVG contains text elements. Cricut may substitute fonts if the font is not available.";
  }

  if (pathCount === 0) {
    return "No path elements were found. The file may still be valid SVG, but it may not behave like a cut-file in Cricut.";
  }

  return null;
}

function getCricutNote(cleaned: {
  hasRasterImage: boolean;
  hasText: boolean;
  pathCount: number;
}) {
  if (cleaned.hasRasterImage) {
    return "Contains embedded raster images. Good for print then cut, not ideal for pure vinyl cut paths.";
  }

  if (cleaned.hasText) {
    return "Contains live text. Install the matching font or convert text to outlines before final Cricut use.";
  }

  if (cleaned.pathCount > 0) {
    return "Contains SVG paths. Good candidate for Cricut cut-file use.";
  }
...
function buildClientLayeredSvg({
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Layered SVG from Base64 image for Cricut">${background}${body}</svg>`;
}
        ...
        <article className="max-w-none">
          ...
          <section className="mt-12">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {[
                ...
                "HTML embedded SVG",
                "CSS background SVG",
                "Print then cut checks",
                "Vinyl cut file checks",
                "Copy SVG",
                "Download SVG",
                "SVG safety cleanup",
                "SVG preview",
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-5">
                ...
                <p className="mt-1 text-sm leading-6 text-slate-600">
                  ...
                  layered SVG paths for Cricut-style projects.
                </p>
              </div>

              <div className="rounded-2xl border border-slate-200 p-5">
                <div className="text-sm font-semibold text-sky-950">
                  Useful before Cricut upload
                </div>
                <p className="mt-1 text-sm leading-6 text-slate-600">
                  Use the report to check whether the decoded file contains
                  paths, embedded raster images, live text, or scripts. Paths
                  are usually better for Cricut cut files than live text or
                  embedded images.
                </p>
              </div>
            </div>
            ...
            <ol className="mt-4 grid gap-3">
              {[
                ...
                },
                {
                  ...
                  body: "Use clean Cricut SVG for most files, single-color cut file for vinyl, or strict clean SVG when you want to remove embedded raster images.",
                },
                {
                  title: "Review the Cricut checks",
                  body: "Check whether the SVG contains paths, text, scripts, metadata, or raster images before uploading it to Cricut Design Space.",
                },
                {
                  title: "Adjust cleanup settings",
                  body: "Preserve colors, force a single cut color, remove scripts, remove metadata, normalize viewBox, or add a preview background.",
                },
                {
          ...
          <section className="mt-12">
            ...
            <p className="mt-2 max-w-[80ch] text-sm leading-6 text-slate-600">
              ...
              viewBox values, or sizing attributes that are awkward in Cricut
              Design Space.
            </p>
              ...
              {[
                {
                  ...
                  body: "Keeps the SVG as close as possible to the decoded source. Best for print then cut or multi-color artwork.",
                },
                {
                  title: "Force single fill color",
                  body: "Turns shapes into one solid color, which is useful for basic vinyl cut files.",
                },
                {
                  title: "Force outline stroke",
                  body: "Shows paths as outlines so you can inspect the cut shape before downloading.",
                },
                {
          ...
          >
            ...
            <div className="mt-4 grid gap-3">
              {[
                {
                  q: "Can I convert Base64 to SVG for Cricut?",
                  a: "Yes. If the Base64 content is encoded SVG, this tool decodes it, cleans it, previews it, and lets you download a normal SVG file for Cricut Design Space.",
                },
                {
                  q: "Does this convert Base64 PNG or JPG into layered SVG paths?",
                  a: "Yes. When the input is a Base64 PNG, JPG, JPEG, or WEBP image, the tool traces the image into simplified color-separated SVG layers for Cricut-style workflows. Very detailed photos may still need simpler settings or a cleaner source image.",
                },
                {
                ...
                {
                  q: "Why does Cricut change the font?",
                  a: "If the decoded SVG contains live text, Cricut may substitute the font. Use installed fonts or convert text to outlines before final cutting.",
                },
                {
                  q: "Should I remove embedded raster images?",
                  a: "Remove them for cleaner vector-only SVG output. Keep them if your project is intended for print then cut.",
                },
                {
                ...
                },
                {
                  ...
                  a: "Yes. Use the single-color cut file preset or choose force single fill color in the settings.",
                },
                {
