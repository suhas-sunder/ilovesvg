...
export default function Base64ToSvgForCricut({}: Route.ComponentProps) {
  ...
  const [activePreset, setActivePreset] =
    React.useState<string>("clean-cricut");
  ...
  function copySvg() {
    ...
    navigator.clipboard.writeText(decoded.svg).then(
  ...
  function copyDecodedText() {
    ...
    navigator.clipboard.writeText(decoded.svg).then(
      () => showToastMessage("Decoded SVG copied"),
  ...
  function downloadSvg() {
    ...
    const blob = new Blob([decoded.svg], {
    ...
    link.download = makeDownloadName(
      settings.filename || "cricut-design",
      "svg",
    );
          ...
          <section className="grid grid-cols-1 items-start gap-4 sm:pt-6 md:grid-cols-2 lg:pb-8 lg:pt-0">
            <div className="min-w-0 overflow-hidden rounded-xl bg-white p-4 sm:border sm:border-slate-200 sm:shadow-sm">
              ...
              <p className="mb-4 text-center text-sm leading-6 text-slate-600">
                Paste a Base64 SVG string, SVG data URL, or encoded SVG snippet
                and turn it into a clean downloadable SVG file for Cricut Design
                Space.
              </p>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <textarea
                  ...
                  rows={9}
                  spellCheck={false}
                  placeholder="Paste data:image/svg+xml;base64,... or plain Base64 SVG here"
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <Field label="File name">
                    <input
                      ...
                      className="w-full rounded-md border border-[#dbe3ef] bg-white px-2 py-1.5 text-slate-900"
                    />
                  </Field>
              ...
              <div className="mt-3 min-w-0">
                ...
                >
                  <span className="inline-flex items-center gap-2">
                    Decode, clean, and Cricut settings
                  </span>
              ...
              {showTips && (
                <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-700">
                  <ul className="list-disc space-y-1 pl-5">
                    <li>
                      This tool is best for Base64-encoded SVG, not Base64 PNG
                      or JPG.
                    </li>
                    <li>
                      If the decoded content is a PNG or JPG, it is still raster
                      artwork and may not become a true Cricut cut file.
                    </li>
                    <li>Use single-color mode for basic vinyl cut files.</li>
                    <li>Keep colors preserved for print then cut artwork.</li>
                    <li>
                      Remove embedded raster images if you want a cleaner
                      vector-only SVG.
                    </li>
                  </ul>
                </div>
...
function decodeAndCleanSvg(settings: Settings): DecodedResult {
  ...
  if (
    ...
    const wrapped = wrapRasterAsSvg(parsed.content, settings);
    const displaySvg = svgToDataUrl(wrapped.svg);
    ...
    return {
      ...
      error: null,
      warning:
        "This input appears to be a Base64 raster image. The output wraps the image inside an SVG, but it is not a true vector cut file. For real Cricut cut paths, use a PNG/JPG to SVG tracing tool.",
      width: wrapped.width,
      height: wrapped.height,
      viewBox: `0 0 ${wrapped.width} ${wrapped.height}`,
      byteSize: wrapped.svg.length,
      elementCount: 2,
      pathCount: 0,
      hasRasterImage: true,
      hasScript: false,
      hasText: false,
      note: "Raster image wrapped in SVG. Not a true vector cut file.",
    };
  }
...
function wrapRasterAsSvg(dataUrl: string, settings: Settings) {
  const width = 1024;
  const height = 1024;
  const bg =
    settings.backgroundMode === "solid"
...
<title>Raster image wrapped as SVG</title>
${bg}
<image href="${escapeXml(dataUrl)}" x="0" y="0" width="${width}" height="${height}" preserveAspectRatio="xMidYMid meet"/>
</svg>`;

  return {
    svg: settings.minifyOutput ? minifySvg(svg) : svg,
    width,
    height,
  };
}
  ...
  return (
    <div className="mb-2 mt-[.67rem] min-w-0">
      ...
      {showToggle && (
        ...
        >
          {expanded ? "Show fewer Base64 presets" : "Show more Base64 presets"}
        </button>
      )}
    </div>
  );
        ...
        <article className="max-w-none">
          ...
          <section className="mt-12">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-5">
                <div className="text-sm font-semibold text-sky-950">
                  Best for SVG Base64, not raster Base64
                </div>
                <p className="mt-1 text-sm leading-6 text-slate-600">
                  This page is best when the encoded content is actually SVG. If
                  the Base64 string is a PNG, JPG, or WEBP, this tool can wrap
                  it inside an SVG, but that does not create true vector cut
                  paths.
                </p>
              </div>
          ...
          <section className="mt-12">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {[
                [
                  "The result says raster image",
                  "The Base64 content is probably PNG, JPG, or WEBP. Use an image-to-SVG tracing tool if you need true cut paths.",
                ],
                [
          ...
          >
            ...
            <div className="mt-4 grid gap-3">
              {[
                ...
                {
                  q: "Does this convert Base64 PNG to a true SVG cut file?",
                  a: "No. A Base64 PNG or JPG is still raster artwork. This tool can wrap it inside an SVG, but that is not the same as vector tracing or cut-path conversion.",
                },
                {
                  q: "What input formats does this accept?",
                  a: "It accepts raw SVG markup, SVG data URLs, Base64-encoded SVG strings, and common raster image data URLs for detection and warning.",
                },
                {
