...
async function traceActionImplementation({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (!file || typeof file === "string") {
      return responseJson({ error: "No JPG file uploaded." }, { status: 400 });
    }
...
export default function JpgToLayeredSvgForCricut({
  ...
  const fetcher = useHybridTraceFetcher<ServerResult>({ routeId: "jpg-to-layered-svg-for-cricut" });
  ...
  React.useEffect(() => {
    ...
    if (fetcher.data.code === "BUSY" && (submitted || lastSubmitRef.current)) {
      ...
      const retryPayload = submitted || lastSubmitRef.current!;
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start sm:pt-6 lg:pt-0 lg:pb-8">
            ...
            <div className="order-1 min-w-0 overflow-hidden rounded-2xl bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_rgba(15,23,42,0.04)] sm:border sm:border-slate-200">
              <h1 className="inline-flex text-center w-full justify-center mb-3 text-sky-950 items-center gap-2 text-xl sm:text-3xl font-extrabold leading-none m-0">
                <CurrentRouteTitle fallback="JPG to Layered SVG for Cricut" />
              </h1>
            ...
            <BespokeTraceOutputPanel
              ...
              downloadFileName="jpg-to-layered-svg-for-cricut.svg"
