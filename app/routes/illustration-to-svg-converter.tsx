...
import type { Route } from "./+types/illustration-to-svg-converter";
import {
  json,
...
======================== */
const ROUTE_PATH = "/illustration-to-svg-converter";
const ROUTE_NAME = "Illustration to SVG Converter";
const BRAND = "i🩵SVG";
...
export function meta({}: Route.MetaArgs) {
  ...
  const description =
    "Convert illustrations and drawings to SVG with presets for clean outlines, painted art edge extraction, and scan cleanup. Live preview, optional white-ink-on-dark output, and privacy-friendly in-memory processing.";
  ...
  return [
    ...
    { name: "theme-color", content: "#0b2dff" },
    {
      name: "keywords",
      content:
        "illustration to svg, drawing to svg, artwork to svg, vectorize illustration, convert drawing to vector, potrace illustration",
    },
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    try {
      ...
      const preprocess = String(form.get("preprocess") ?? "edge") as "none" | "edge";
      ...
      const finalSVG = transparent ? svg3 : injectBackgroundRectString(svg3, ensured.width, ensured.height, bgColor);
...
function ensureViewBoxResponsive(svg: string): { svg: string; width: number; height: number } {
  ...
  if (!hasViewBox) {
    newOpen = newOpen.replace(/<svg\b/i, `<svg viewBox="0 0 ${Math.round(width)} ${Math.round(height)}"`);
  }
...
const FAQ: FaqItem[] = [
  {
    q: "What illustrations work best for SVG conversion?",
    a: "Drawings with clear outlines and strong contrast work best. For painted or textured art, use Edge mode to extract contours.",
  },
  {
    q: "How do I get cleaner outlines from a painting?",
    a: "Use Edge mode, increase edge boost, and adjust blur to suppress texture. Then raise curve tolerance to smooth the resulting vectors.",
  },
  {
    q: "My result has too many tiny bits or dots. How do I remove them?",
    a: "Increase turd size to remove speckles. For noisy scans, add a little blur in Edge mode.",
  },
  {
    q: "Why is the SVG blank or too light?",
    a: "Lower the threshold to capture more ink, or disable invert. If your art is light, Edge mode usually works better than None.",
  },
];
...
/**
 * Illustration-focused presets:
 * Emphasize contour extraction + clean outlines for drawings and painted art.
 */
const PRESETS: Preset[] = [
  {
    id: "illustration-clean-outline",
    label: "Illustration: Clean outlines (default)",
    settings: {
      preprocess: "edge",
      blurSigma: 0.9,
      edgeBoost: 1.1,
  ...
  {
    id: "drawing-lineart",
    label: "Drawing: Lineart (already clean)",
    settings: {
      preprocess: "none",
      threshold: 224,
      turdSize: 2,
  ...
  {
    id: "painted-edges-soft",
    label: "Painting: Soft contour extraction",
    settings: {
      preprocess: "edge",
      blurSigma: 1.2,
      edgeBoost: 0.95,
  ...
  {
    id: "painted-edges-bold",
    label: "Painting: Bold contour extraction",
    settings: {
      preprocess: "edge",
      blurSigma: 0.7,
      edgeBoost: 1.5,
...
function autoModeHint(mode: AutoMode): string {
  if (mode === "medium") return "Live preview is throttled for 10 to 25 MB files.";
  return "";
}
...
export default function IllustrationToSvgConverter({ loaderData }: Route.ComponentProps) {
  const fetcher = useFetcher<ServerResult>();
  const [file, setFile] = React.useState<File | null>(null);
  const [originalFileSize, setOriginalFileSize] = React.useState<number | null>(null);
  const [previewUrl, setPreviewUrl] = React.useState<string | null>(null);
  const [settings, setSettings] = React.useState<Settings>(DEFAULTS);
  const [activePreset, setActivePreset] = React.useState<string>("illustration-clean-outline");
  ...
  async function handleNewFile(f: File) {
    ...
    setActivePreset("illustration-clean-outline");
    setHistory([]);
  ...
  function applyPreset(preset: Preset) {
    ...
    setSettings((s) => {
      const baseline: Settings = { ...DEFAULTS, transparent: s.transparent, bgColor: s.bgColor };
  ...
  return (
    <>
      ...
      <main className="min-h-[100dvh] bg-slate-50 text-slate-900">
        <div className="max-w-[1180px] mx-auto px-4 pt-6 pb-12">
          ...
          <header className="text-center mb-2">
            ...
            <p className="mt-1 text-slate-600 max-w-[85ch] mx-auto">
              Convert drawings, illustrations, and painted artwork into editable SVG paths.
              Use Lineart mode for already-clean drawings, or Edge mode to extract contours from textured art.
            </p>
          </header>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm overflow-hidden min-w-0">
              ...
              <div className="text-sky-700 mb-2 text-center text-sm">
                Live preview: fast ≤10 MB, throttled ≤25 MB. Files over 30 MB are auto-compressed on-device (if possible).
              </div>
              ...
              {!file ? (
                ...
                >
                  <div className="text-sm text-slate-600">Click, drag & drop, or paste an illustration (PNG/JPEG)</div>
                  <input
                    id="file-inp"
                    type="file"
                ...
                <>
                  <div className="flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-[#f7faff] border border-[#dae6ff] text-slate-900 mt-0">
                    <div className="flex items-center min-w-0 gap-2">
                      {previewUrl && (
                        <img src={previewUrl} alt="" className="w-[22px] h-[22px] rounded-md object-cover mr-1" />
                      )}
              ...
              <div className="mt-3 flex flex-col gap-2 min-w-0">
                <Field label="Preprocess">
                  ...
                  >
                    <option value="none">None (clean drawing)</option>
                    <option value="edge">Edge (painting / textured art)</option>
                  </select>
                </Field>
                ...
                {settings.preprocess === "edge" && (
                  <>
                    <Field label={`Blur σ (${settings.blurSigma})`}>
                      <Num value={settings.blurSigma} min={0} max={3} step={0.1} onChange={(v) => setSettings((s) => ({ ...s, blurSigma: v }))} />
                    </Field>
                    <Field label={`Edge boost (${settings.edgeBoost})`}>
                      <Num value={settings.edgeBoost} min={0.5} max={2.0} step={0.1} onChange={(v) => setSettings((s) => ({ ...s, edgeBoost: v }))} />
                    </Field>
                  </>
                )}
                ...
                <Field label="Curve tolerance (smoothness)">
                  <Num value={settings.optTolerance} min={0.05} max={1.2} step={0.05} onChange={(v) => setSettings((s) => ({ ...s, optTolerance: v }))} />
                </Field>
      ...
      <SeoSectionsIllustration faq={FAQ} />
...
/* ========================
   Route-specific SEO + rendered FAQ
======================== */
function SeoSectionsIllustration({ faq }: { faq: FaqItem[] }) {
  return (
      ...
      <div className="max-w-[1180px] mx-auto px-4 py-12 text-slate-800">
        <article className="max-w-none">
          <header className="rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-8">
            <p className="text-xs font-semibold tracking-wide text-slate-500 uppercase">Illustration vectorizer</p>
            <h2 className="text-2xl md:text-3xl font-bold leading-tight">Convert illustrations into clean SVG outlines</h2>
            <p className="text-slate-600 max-w-[85ch]">
              Use this tool when you want scalable vectors from drawings, illustrations, or painted art.
              It supports two workflows: direct tracing for clean line drawings and contour extraction for textured art.
            </p>

            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                { k: "Outlines first", v: "Designed for drawings and illustrations" },
                { k: "Edge extraction", v: "Works on paintings and textured art" },
                { k: "Live preview", v: "Fast ≤10 MB, throttled ≤25 MB" },
                { k: "Private", v: "Processed in memory, no accounts" },
              ].map((x) => (
          ...
          <section className="mt-10">
            <h3 className="text-lg font-bold">How to convert an illustration to SVG</h3>
            <ol className="mt-4 grid gap-3">
              {[
                { title: "Upload your artwork", body: "PNG or JPEG. Crop extra margins to reduce noise and speed up conversion." },
                { title: "Pick a preset", body: "Use Clean outlines for general artwork, Lineart for already-inked drawings, and Painting presets for contour extraction." },
                { title: "Adjust Edge mode for textured art", body: "Increase blur to suppress texture and increase edge boost to emphasize contours." },
                { title: "Tune threshold and smoothness", body: "Threshold controls what becomes ink; curve tolerance controls how smooth and lightweight the SVG becomes." },
                { title: "Export your SVG", body: "Download or copy the SVG. Refine paths in Illustrator/Inkscape if you need fewer nodes or sharper geometry." },
              ].map((s, i) => (
          ...
          <section className="mt-12">
            <h3 className="text-lg font-bold">Tips for better results</h3>
            <div className="mt-4 grid md:grid-cols-2 gap-4">
              {[
                { title: "For clean drawings", body: "Use None preprocess. Lower curve tolerance for detail, raise it for smoother paths." },
                { title: "For paintings", body: "Use Edge preprocess. Start with blur ~0.9 and edge boost ~1.1, then increase edge boost if outlines are weak." },
                { title: "Too many speckles", body: "Increase turd size. If noise is from texture, increase blur slightly in Edge mode." },
                { title: "Too many nodes", body: "Increase curve tolerance a bit. Fewer nodes, smoother shapes, smaller SVG." },
              ].map((x) => (
