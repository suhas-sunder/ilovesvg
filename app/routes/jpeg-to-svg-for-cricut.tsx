...
import type { Route } from "./+types/jpeg-to-svg-for-cricut";
import {
  json,
...
export function meta({}: Route.MetaArgs) {
  const title = "JPEG to SVG for Cricut - Free Cricut SVG Converter | iLoveSVG";
  const description =
    "Convert JPEG images to clean SVG files for Cricut projects. Make cut-friendly SVGs for vinyl decals, stickers, logos, handwriting, and simple craft designs.";
  const canonical = "https://www.ilovesvg.com/jpeg-to-svg-for-cricut";
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    try {
      ...
      // Force sensible output for white-on-dark in single-color mode only
      if (whiteOnDark) {
        transparent = false;
        if (
          !bgColor ||
...
const PRESETS: Preset[] = [
  ...
  {
    id: "remove-speckles",
    label: "Cleanup - Remove JPEG speckles",
    settings: {
      traceMode: "single",
      preprocess: "none",
      threshold: 226,
  ...
  {
    id: "low-contrast-jpg",
    label: "Low contrast JPEG - Boost edges",
    settings: {
      traceMode: "single",
      preprocess: "edge",
      blurSigma: 1.0,
...
const routeCapabilities = getRouteCapabilities("jpeg-to-svg-for-cricut");
...
export default function Home({ loaderData }: Route.ComponentProps) {
  getRasterToSvgRouteContextByKey("jpeg-cricut");
  const fetcher = useHybridTraceFetcher<ServerResult>({ routeId: "jpeg-to-svg-for-cricut" });
  ...
  const [activePreset, setActivePreset] =
    React.useState<string>("jpeg-cricut-clean-cut");
  ...
  async function handleNewFile(f: File) {
    ...
    if (!isSvgFile(chosen) && chosen.size > LIVE_MED_MAX) {
      try {
    ...
    // Re-enable live preview and submit the selected file directly so the first
    // upload never depends on stale React state.
  ...
  async function submitConvertWith(
    ...
    const effective = (() => {
      if (!targetSettings.invert || targetSettings.traceMode === "layered") return targetSettings;
      const bg =
        ...
        <div className="max-w-[1180px] mx-auto px-4">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start sm:pt-6 lg:pt-0 lg:pb-8">
            ...
            <TraceOutputPanel
              ...
              downloadFileName="jpeg-to-svg-for-cricut.svg"
        ...
        <article className="max-w-none">
          <header className="rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-[0_1px_2px_rgba(15,23,42,0.04)] md:p-8">
            <div className="flex flex-col gap-3">
              ...
              <p className="text-slate-600">
                Turn a JPEG into a simple SVG cut file for Cricut Design Space.
              ...
              <p className="text-slate-600">
                ...
                image. Upload your JPEG, choose a preset, adjust the trace, then
              ...
              <div className="mt-2 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  ...
                  {
                    k: "Cleanup controls",
                    v: "Reduce JPEG speckles, rough edges, and tiny artifacts",
                  },
                  {
                    k: "Fast preview",
            ...
            <div className="mt-4 grid md:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-slate-200 p-5">
                ...
                <p className="mt-1 text-sm text-slate-600">
                  JPEG images with clear edges, dark shapes, and plain
              ...
              {[
                {
                  ...
                  body: "Start with the clearest version you have. A high-contrast JPEG with a plain background will usually produce a cleaner Cricut SVG.",
                },
                {
                ...
                },
                {
                  ...
                  body: "Use threshold to control what becomes solid, turd size to remove tiny JPEG speckles, and curve tolerance to balance detail against smoother cutting paths. Advanced setting changes apply when you click Convert or Update preview.",
                },
                {
          ...
          <section className="mt-12">
            ...
            <p className="mt-2 text-sm text-slate-600 max-w-[80ch]">
              Different JPEG images need different tracing behavior. Use the
              preset that matches what you are trying to cut. Start with the
              layered color presets for separated color designs, then use the
              single-color presets for silhouettes, line art, cleanup, and
              simpler cut files.
            </p>
              ...
              {[
                {
                  ...
                  body: "Best starting point when you want separated color layers for Cricut-style designs, stickers, and simple multi-color graphics.",
                },
                {
                  title: "Layered color SVG - Smoother",
                  body: "Use this when the preview has too many tiny color regions and you want fewer, smoother layers.",
                },
                {
                  title: "Layered color SVG - More detail",
                  body: "Use this when small color areas matter and the design needs a more detailed layered trace.",
                },
                {
                  title: "Cricut - Clean cut file",
                  body: "Best for simple single-color craft graphics, dark shapes, and images that already look close to a cut file.",
                },
                {
                ...
                {
                  title: "Cleanup - Remove JPEG speckles",
                  body: "Use this when the JPEG has compression dots, scanner dust, or small unwanted marks.",
                },
                {
                ...
                },
                {
                  ...
                  body: "Use this for a stylized single-color outline from a photo when a layered color result is not the goal.",
                },
              ].map((c) => (
                <div
                  key={c.title}
            ...
            <div className="mt-5 grid md:grid-cols-2 gap-4">
              {[
                ...
                },
                {
                  ...
                  body: "If the JPEG has shadows, furniture, paper texture, or background objects, the converter may trace those too.",
                },
                {
                ...
                },
                {
                  ...
                  body: "JPEG compression creates small artifacts. Higher turd size removes more of those tiny unwanted marks.",
                },
                {
              ...
              {[
                {
                  ...
                  a: "Yes. Cricut Design Space supports SVG uploads. This tool creates a simple traced SVG that is meant for cut-style projects, especially when your original file is a JPEG.",
                },
                {
                  q: "Is this better than uploading a JPEG directly to Cricut?",
                  a: "It depends on the project. Uploading the JPEG directly may be fine for Print Then Cut, but an SVG is usually better when you want scalable vector shapes for simple cut files, decals, labels, and silhouettes.",
                },
                {
                  q: "Will this make a layered multi-color Cricut SVG?",
                  a: "Yes. Use the layered color SVG presets when you want separated color layers for Cricut-style designs. Single-color presets remain available for silhouettes, line art, decals, and cleanup work.",
                },
                {
                  q: "Why does my JPEG make a rough SVG?",
                  a: "JPEG files often contain compression artifacts, shadows, and blurry edges. Use a higher-quality image, remove the background first, or try the cleanup presets.",
                },
                {
