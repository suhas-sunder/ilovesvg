...
const routeCapabilities = getRouteCapabilities("webp-to-svg-for-cricut");
...
export default function Home({ loaderData }: Route.ComponentProps) {
  getRasterToSvgRouteContextByKey("webp-cricut");
  const fetcher = useHybridTraceFetcher<ServerResult>({ routeId: "webp-to-svg-for-cricut" });
  ...
  const [activePreset, setActivePreset] =
    React.useState<string>("webp-cricut-clean-cut");
  ...
  // When a new server SVG arrives, push a new card or replace the targeted card.
  React.useEffect(() => {
    if (fetcher.data?.svg) {
      const resultKey = `${fetcher.data.svg}:${fetcher.data.width ?? ""}:${
        fetcher.data.height ?? ""
      }`;
  ...
  function submitOutputUpdate(stamp: number) {
    ...
    void submitConvert(file, nextSettings, stamp, item.presetId ?? activePreset);
  }
        ...
        <div className="max-w-[1180px] mx-auto px-4">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start sm:pt-6 lg:pt-0 lg:pb-8">
            ...
            <TraceOutputPanel
              ...
              downloadFileName="webp-cricut-cut-file.svg"
            ...
            <div className="mt-5 grid md:grid-cols-2 gap-4">
              {[
                [
                  ...
                  "Use Cleanup - Remove WebP Artifacts or raise turd size in settings.",
                ],
                [
