...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    if (!rateLimit.allowed) {
      const blocked = rateLimit as Extract<
        BackendRateLimitResult,
        { allowed: false }
      >;
      return json(
        {
          error: `Too many conversions from this connection. Please try again in ${blocked.retryAfterText}.`,
...
// Existing JPEG presets are preserved exactly and moved below the new layered color presets.
...
const routeCapabilities = getRouteCapabilities("jpeg-to-svg-converter");
...
export default function JpegToSvgConverter({}: Route.ComponentProps) {
    ...
    const effective = (() => {
      if (!targetSettings.invert || targetSettings.traceMode === "layered") {
        return {
          ...targetSettings,
          invert:
            targetSettings.traceMode === "layered"
              ? false
              : targetSettings.invert,
        };
      }
      const bg =
  ...
  React.useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      void submitConvertWith(file, settings);
    }, delay);
...
function ThrottledColorSettingInput({
  value,
  onChange,
  disabled = false,
  title,
  ...
  function schedule(nextColor: string) {
  ...
  return (
    <input
      ...
      className={[
        "w-14 h-7 rounded-md border border-[#dbe3ef] bg-white cursor-pointer transition-opacity hover:opacity-90",
