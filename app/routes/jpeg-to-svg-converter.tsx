...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    // Early reject: do not parse multipart if request is huge
    ...
    try {
      ...
      // Potrace params (defaults aligned with "safe" baseline)
      ...
      // Preprocess (safe default is none; presets can switch to edge)
...
async function normalizeForPotrace(
  ...
  try {
    ...
    // If sharp is not available or fails, return original
    return input;
  }
}
...
// "Safe like home page" means DEFAULTS are conservative (preprocess none).
// JPEG photo outlines can still be done by selecting one of the Edge presets below.
const PRESETS: Preset[] = [
  {
    id: "scan-clean",
    label: "JPG Scan - Clean (remove speckles)",
    settings: {
      preprocess: "none",
      threshold: 226,
      turdSize: 4,
  ...
  {
    id: "scan-aggressive",
    label: "JPG Scan - Aggressive (close gaps)",
    settings: {
      preprocess: "none",
      threshold: 218,
      turdSize: 5,
  ...
  },
  {
    ...
    label: "JPG Photo Edge - Normal outline",
    settings: {
      preprocess: "edge",
      blurSigma: 0.9,
      edgeBoost: 1.1,
  ...
  {
    id: "photo-soft",
    label: "JPG Photo Edge - Soft contour",
    settings: {
      preprocess: "edge",
      blurSigma: 1.2,
      edgeBoost: 0.9,
  ...
  {
    id: "photo-bold",
    label: "JPG Photo Edge - Bold contour",
    settings: {
      preprocess: "edge",
      blurSigma: 0.6,
      edgeBoost: 1.4,
...
function autoModeHint(mode: AutoMode): string {
  if (mode === "medium")
    return "Live preview is throttled for 10 to 25 MB JPEGs.";
  return "";
}
function autoModeDetail(mode: AutoMode): string {
  if (mode === "medium")
    return "Large JPEG, updates run less frequently to keep things smooth.";
  return "";
}
...
export default function JpegToSvgConverter({}: Route.ComponentProps) {
  ...
  async function handleNewFile(f: File) {
    if (!ALLOWED_MIME.has(f.type)) {
      setErr(
        "Please choose a JPEG/JPG (or PNG). This page is optimized for JPEG.",
      );
      return;
    }
          ...
          <section className="lg:pt-0 lg:pb-8 grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            ...
            <div className="bg-white sm:border sm:border-slate-200 rounded-xl p-4 sm:shadow-sm overflow-hidden min-w-0">
              ...
              <div className="flex items-center gap-3 mt-3 flex-wrap">
                ...
                >
                  <Icons name="convert" size={18} className="mr-1" />
                  {busy ? "Converting..." : "Convert JPEG to SVG"}
                </button>
                    ...
                    >
                      ...
                      <div className="flex gap-3 items-center mt-3 flex-wrap justify-between">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          >
                            <Icons name="download" size={18} className="mr-2" />
                            Download SVG
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            <Icons name="copy" size={18} className="mr-2" />
                            Copy SVG
                          </button>
                        </div>
                      </div>
...
/** Compress to ≤25MB (best effort). Keeps JPEG output. */
async function compressToTarget25MB(file: File): Promise<File> {
  ...
  throw new Error("This image cannot be reduced below 25 MB.");
}
...
/* ========================
   JPEG-specific SEO (unique content)
======================== */
function JpegSeoSections() {
  return (
    <section className="bg-white border-t border-slate-200">
      <div className="max-w-[1180px] mx-auto px-4 py-8 text-slate-800">
        <article className="max-w-none">
          <header className="rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-8">
            ...
            <p className="mt-2 text-slate-600 max-w-[85ch]">
              JPEG is the format you get from cameras, screenshots, and most
              scanners. The tricky part is compression: soft gradients and tiny
              artifacts can turn into unwanted dots when you trace. This page is
              built around two clean workflows: scan-style tracing (no
              preprocess) and photo-style contour tracing (edge preprocess).
            </p>

            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                { k: "Scan mode", v: "Best for ink on white" },
                { k: "Photo contours", v: "Edge presets for outlines" },
                { k: "Noise control", v: "Blur and speck cleanup" },
                { k: "Strict limits", v: "30 MB, 30 MP, 8000 px" },
              ].map((x) => (
                <div
                  key={x.k}
            ...
            <div className="mt-3 grid md:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="text-sm font-semibold">
                  If it is a scan or document
                </div>
                <p className="mt-1 text-sm text-slate-600">
                  Use a <b>JPG Scan</b> preset with preprocess set to{" "}
                  <b>None</b>. Raise <b>Turd size</b> to remove speckles. Adjust
                  threshold until the ink looks solid without filling the
                  background.
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="text-sm font-semibold">If it is a photo</div>
                <p className="mt-1 text-sm text-slate-600">
                  Choose a <b>JPG Photo Edge</b> preset. Increase blur to calm
                  noise, then edge boost to strengthen the contour. Keep
                  threshold changes small.
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="text-sm font-semibold">Whiteboard captures</div>
                <p className="mt-1 text-sm text-slate-600">
                  Whiteboards usually have glare and haze. Use <b>Whiteboard</b>{" "}
                  preset, then increase blur slightly if you still see clutter.
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <div className="text-sm font-semibold">
                  If the result looks noisy
                </div>
                <p className="mt-1 text-sm text-slate-600">
                  For scan mode, increase turd size. For edge mode, increase
                  blur first, then reduce edge boost a little. Too much boost
                  pulls compression blocks into the trace.
                </p>
              </div>
            </div>
          </section>
          ...
          >
            ...
            <ol className="mt-4 grid gap-3">
              {[
                {
                  title: "Upload a JPG or JPEG",
                  body: "Photos, scans, and screenshots work. Keep it under 30 MB and within the resolution limits shown above.",
                },
                {
                  title: "Choose scan mode or photo mode",
                  body: "Use Scan presets for ink on white. Use Photo Edge presets for contour-style outlines from photos.",
                },
                {
                  title: "Tune noise first, then threshold",
                  body: "If you see clutter: raise Turd size (scan) or raise Blur (edge). Then adjust Threshold to control what becomes ink.",
                },
                {
                  title: "Pick background behavior",
                  body: "Transparent keeps the SVG easy to overlay. Turn off transparency if you need a solid background baked in.",
                },
                {
                  title: "Download the SVG",
                  body: "Use the SVG in editors, on the web, or in cutting/printing workflows.",
                },
              ].map((s, i) => (
                <li
                  key={s.title}
          ...
          >
            <h3 className="text-lg font-bold">JPEG to SVG FAQ</h3>

            <div className="mt-4 grid gap-3">
              {[
                {
                  q: "Why does JPEG sometimes produce speckles in the SVG?",
                  a: "JPEG compression creates small artifacts and block edges. In scan mode, increase Turd size to remove specks. In edge mode, increase Blur and lower Edge boost slightly.",
                },
                {
                  q: "Should I use Edge mode for every JPG?",
                  a: "No. Edge mode is best when you want a contour outline from a photo. For clean ink scans, preprocess None usually produces a cleaner trace with fewer stray paths.",
                },
                {
                  q: "What sizes are supported?",
                  a: "Up to 30 MB per image, plus a resolution guard around 30 MP and 8000 px per side. Preview is fastest up to 10 MB and throttled up to 25 MB.",
                },
                {
                  q: "Why do I see a server busy message?",
                  a: "Vectorization is CPU heavy. When the conversion queue is full, the server replies with a retry delay and this page retries automatically.",
                },
              ].map((x) => (
                <article
                  key={x.q}
        ...
        {/* Minimal JSON-LD (unique to this page) */}
        <script
            ...
            __html: JSON.stringify(
              {
                ...
                url: "https://www.ilovesvg.com/jpeg-to-svg-converter",
              },
              null,
              2,
            ),
