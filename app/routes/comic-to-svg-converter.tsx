...
import type { Route } from "./+types/comic-to-svg-converter";
import {
  json,
...
======================== */
const ROUTE_PATH = "/comic-to-svg-converter";
const ROUTE_NAME = "Comic to SVG Converter";
const BRAND = "i🩵SVG";
...
export function meta({}: Route.MetaArgs) {
  ...
  const description =
    "Convert comic art and inked panels into SVG. Tuned for bold outlines, black fills, and crisp inking, with options for scan cleanup, gap sealing, and white-ink-on-dark output. Live preview, privacy-friendly in-memory processing.";
  ...
  return [
    ...
    { name: "theme-color", content: "#0b2dff" },
    {
      name: "keywords",
      content:
        "comic to svg, manga to svg, inked art to svg, vectorize comic art, comic panel to svg, potrace comics",
    },
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    try {
      ...
      // Dimension/MP guard via sharp metadata
      try {
        const { createRequire } = await import("node:module");
        const req = createRequire(import.meta.url);
      ...
      const threshold = Number(form.get("threshold") ?? 230);
      const turdSize = Number(form.get("turdSize") ?? 3);
      const optTolerance = Number(form.get("optTolerance") ?? 0.48);
      const turnPolicy = String(form.get("turnPolicy") ?? "black") as
        | "black"
        | "white"
        | "left"
        | "right"
      ...
      const preprocess = String(form.get("preprocess") ?? "edge") as
        | "none"
        | "edge";
      const blurSigma = Number(form.get("blurSigma") ?? 0.7);
      const edgeBoost = Number(form.get("edgeBoost") ?? 1.5);
...
const FAQ: FaqItem[] = [
  {
    q: "What kind of comic art works best?",
    a: "Inked line work with strong contrast works best: black outlines on white paper, clean scans, or exported comic panels.",
  },
  {
    q: "How do I keep thick inks and black fills?",
    a: "Use the Comics: Inks preset, keep preprocess on Edge, and raise threshold to include darker midtones. Increase curve tolerance to smooth chunky inks.",
  },
  {
    q: "My scan has dust and tiny dots. How do I remove them?",
    a: "Increase turd size. If the page is noisy, use Scan cleanup or increase blur slightly in Edge mode.",
  },
  {
    q: "Why do I see Server busy and Retry-After?",
    a: "Vectorization is CPU heavy. We cap concurrent conversions to keep the site stable. The app retries automatically after the suggested delay.",
  },
];
...
/**
 * Comic-focused presets:
 * Keep “chunky inks” and “panel cleanup” front and center.
 */
const PRESETS: Preset[] = [
  {
    id: "comics-inks",
    label: "Comics: Inks (chunky) (default)",
    settings: {
      preprocess: "edge",
      blurSigma: 0.7,
      edgeBoost: 1.5,
  ...
  {
    id: "panel-scan-clean",
    label: "Panel Scan: Clean (remove speckles)",
    settings: {
      preprocess: "none",
      threshold: 226,
      turdSize: 4,
  ...
  {
    id: "panel-gaps",
    label: "Panel: Seal gaps (connect broken lines)",
    settings: {
      preprocess: "none",
      threshold: 218,
      turdSize: 4,
  ...
  },
  {
    ...
    label: "Invert: White ink on dark",
    settings: {
      preprocess: "edge",
      blurSigma: 0.8,
      edgeBoost: 1.4,
...
export default function ComicToSvgConverter({
  loaderData,
  ...
  const [activePreset, setActivePreset] = React.useState<string>("comics-inks");
        ...
        <div className="max-w-[1180px] mx-auto px-4 pt-6 pb-12">
          ...
          <header className="text-center mb-2">
            ...
            <p className="mt-1 text-slate-600 max-w-[80ch] mx-auto">
              Vectorize comic panels, manga inks, and bold linework into clean
              SVG paths. Tuned presets preserve chunky inks, black areas, and
              crisp outlines.
            </p>
          </header>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm overflow-hidden min-w-0">
              ...
              {!file ? (
                ...
                >
                  <div className="text-sm text-slate-600">
                    Click, drag & drop, or paste a PNG/JPEG comic panel
                  </div>
                  <input
                    id="file-inp"
                    type="file"
              ...
              <div className="mt-3 flex flex-col gap-2 min-w-0">
                <Field label="Preprocess">
                  ...
                  >
                    <option value="none">None (already-inked)</option>
                    <option value="edge">Edge (best for comics)</option>
                  </select>
                </Field>
                ...
                <Field label="Invert (white ink on dark)">
                  <input
                    type="checkbox"
...
======================== */
function SeoSectionsComic({ faq }: { faq: FaqItem[] }) {
  return (
        ...
        <article className="max-w-none">
          <header className="rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-8">
            ...
            <h2 className="text-2xl md:text-3xl font-bold leading-tight">
              Turn comic inks into scalable SVG paths
            </h2>
            <p className="text-slate-600 max-w-[80ch]">
              This tool is tuned for comic panels, manga inks, and bold
              outlines. It prioritizes strong contours, solid shapes, and
              smooth, chunky linework so your art stays readable after
              vectorization.
            </p>
            ...
            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                {
                  k: "Comics presets",
                  v: "Chunky inks, crisp outlines, panel cleanup",
                },
                {
                  k: "Solid results",
                  v: "Handles bold blacks and heavy strokes",
                },
          ...
          <section className="mt-10">
            <h3 className="text-lg font-bold">
              How to convert a comic panel to SVG
            </h3>
            <ol className="mt-4 grid gap-3">
              {[
                {
                  title: "Upload a comic panel or inked page",
                  body: "Use PNG or JPEG. Crop margins and remove page borders for cleaner vectors and faster preview.",
                },
                {
                  title: "Start with Comics: Inks",
                  body: "This preset is tuned for bold outlines and heavy inks. Switch to Crisp outlines for cleaner digital panels.",
                },
                {
                  title: "Tune threshold for blacks",
                  body: "Increase threshold to keep blacks solid. If the result becomes too filled, back off slightly.",
                },
                {
                  title: "Cleanup speckles with turd size",
                  body: "Scans often have dust. Raise turd size to remove tiny dots and paper noise.",
                },
                {
                  title: "Export and refine",
                  body: "Download the SVG, then refine in Illustrator/Inkscape. For web use, you can also recolor or compress paths later.",
                },
              ].map((s, i) => (
                <li
                  key={s.title}
          ...
          <section className="mt-12">
            ...
            <div className="mt-4 grid md:grid-cols-2 gap-4">
              {[
                {
                  title: "Jagged inks",
                  body: "Increase curve tolerance slightly. If edges get noisy, add a bit more blur in Edge mode.",
                },
                {
                  title: "Broken outlines",
                  body: "Use Panel: Seal gaps and set turn policy to black to connect corners.",
                },
                {
                  title: "Too much paper texture",
                  body: "Increase turd size. If needed, increase blur σ a bit to reduce grain before tracing.",
                },
                {
                  title: "Looks too thin",
                  body: "Increase threshold and edge boost to emphasize outlines before tracing.",
                },
              ].map((x) => (
                <div
                  key={x.title}
